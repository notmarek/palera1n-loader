...
struct ContentView: View {
                ...
                content
                    .onAppear {
                        ...
                        self.splashTimeout = Timer.scheduledTimer(withTimeInterval: 1, repeats: false) { _ in
                            ...
                            console.log("Welcome to pokem0n loader")
                            console.log(uname())
        ...
        let task = session.downloadTask(with: url) { tempLocalUrl, response, error in
            if let tempLocalUrl = tempLocalUrl, error == nil {
                do {
                    ...
                    print("[pokem0n] Could not copy file to disk: \(writeError)")
                }
            } else {
                self.console.error("[-] Could not download file: \(error?.localizedDescription ?? "Unknown error")")
                tb.toolbarState = .closeApp
                print("[pokem0n] Could not download file: \(error?.localizedDescription ?? "Unknown error")")
            }
        }
        task.resume()
        semaphore.wait()
    ...
    private func strap() -> Void {
        ...
        guard let helper = Bundle.main.path(forAuxiliaryExecutable: "pokem0nHelper") else {
                                    ...
                                    DispatchQueue.main.async {
                                        ...
                                        DispatchQueue.global(qos: .utility).async {
                                            ...
                                            DispatchQueue.main.async {
                                                ...
                                                console.log("[*] Installing pokem0n strap repo")
                                                DispatchQueue.global(qos: .utility).async {
                                                    let ret = spawn(command: "/usr/bin/dpkg", args: ["-i", strapRepo], root: true)
                                                    DispatchQueue.main.async {
                                                        if ret != 0 {
                                                            console.error("[-] Failed to install pokem0n strap repo. Status: \(ret)")
                                                            tb.toolbarState = .closeApp
                                                            return
                                                        }
